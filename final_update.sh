#!/bin/bash
set -e
echo "🔄 Полное обновление админ-панели (загрузка фото + отдельные поля)"

# ========== 1. Обновляем модели ==========
echo "📁 Обновляю модели..."
...
# ========== 2. Обновляем admin.py с правильной загрузкой ==========
echo "📁 Обновляю admin.py..."

cat > Backend/app/admin.py << 'ADMIN_EOF'
...
# Базовый класс с общей логикой загрузки фото
...
# Для всех брендов с полем main_image используем BaseProductAdmin
...
echo "✅ admin.py обновлён с загрузкой фото через FileInput"

# ========== 3. Добавляем колонки в БД ==========
echo "🔄 Добавляю колонки в базу данных..."
...
with engine.connect() as conn:
    # Добавляем новые поля в таблицу products, если их нет
    for col in ['description', 'color', 'width', 'height', 'depth', 'weight']:
        try:
            conn.execute(text(f'ALTER TABLE products ADD COLUMN {col} TEXT;'))
            print(f'✅ Колонка {col} добавлена в products')
        except Exception as e:
            if 'duplicate column' in str(e).lower():
                print(f'ℹ️ Колонка {col} уже есть в products')
            else:
                print(f'⚠️ Ошибка при добавлении {col}: {e}')
    # Добавляем main_image в Liebherr и Teka
    for table in ['products_liebherr', 'products_teka']:
        try:
            conn.execute(text(f'ALTER TABLE {table} ADD COLUMN main_image VARCHAR(500);'))
            print(f'✅ Колонка main_image добавлена в {table}')
        except Exception as e:
            if 'duplicate column' in str(e).lower():
                print(f'ℹ️ Колонка main_image уже есть в {table}')
            else:
                print(f'⚠️ Ошибка при добавлении main_image в {table}: {e}')
    conn.commit()
"
cd ..
