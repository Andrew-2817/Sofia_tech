#!/usr/bin/env python3
"""Извлечение изображений из Excel файла и загрузка товаров Falmec в БД"""
...
EXCEL_FILE = BASE_DIR / 'Backend' / 'app' / 'zavoz' / 'exelfiles' / 'файл_товары_10.xlsx'
...
from Backend.app.models.product_falmec import FalmecProduct
...
def clean_price(price_str):
    ...
    price_str = price_str.replace('₽', '').replace('руб', '').replace('р', '').replace(' ', '').replace(',', '.')
...
def clean_float(value):
    """Очистка числа с плавающей точкой"""
    if pd.isna(value):
        return None
    try:
...
def clean_integer(value):
    ...
    try:
        if isinstance(value, str):
            # Извлекаем первое число из строки типа "450/720" или "56 Дб"
...
def clean_width(value):
    """Очистка ширины (может быть '66', 'Ø 54', '36*90')"""
    if pd.isna(value):
        return None
    try:
        value_str = str(value).strip()
        # Убираем символы Ø, * и т.д.
        value_str = re.sub(r'[Øø*]', '', value_str)
        # Берем первое число
...
def cleanup_products_table():
    """Очистка таблицы products_falmec перед загрузкой"""
    print("=" * 70)
    print("🗑️ ОЧИСТКА ТАБЛИЦЫ products_falmec")
    print("=" * 70)

    db = SessionLocal()
    try:
        count_before = db.query(FalmecProduct).count()
        print(f"📊 Записей до очистки: {count_before}")

        deleted = db.query(FalmecProduct).delete()
        db.commit()

        try:
            db.execute(text("ALTER SEQUENCE products_falmec_id_seq RESTART WITH 1"))
            db.commit()
        except:
            pass

        print(f"✅ Удалено записей: {deleted}")
        print(f"📊 Записей после очистки: {db.query(FalmecProduct).count()}")
...
def extract_images_by_sheet():
    ...
    print("=" * 70)
    print("📸 ИЗВЛЕЧЕНИЕ ИЗОБРАЖЕНИЙ ПО ЛИСТАМ (FALMEC)")
    print("=" * 70)
    ...
    # Очищаем старые фото Falmec
    for old_file in PHOTOS_DIR.glob("10.*.*"):
        old_file.unlink()
    for old_file in UPLOADS_DIR.glob("10.*.*"):
        old_file.unlink()
    print("📁 Старые фото Falmec удалены")
                                    ...
                                    if r_id in rid_to_image:
                                        ...
                                        if img_path_in_zip in all_files:
                                            ...
                                            if len(img_data) > 100:
                                                ...
                                                img_name = f"10.{sheet_idx}.{xml_row}.{ext}"
...
def load_products_to_db(photos_by_sheet):
    """Загрузка товаров Falmec в базу данных"""
    print("\n" + "=" * 70)
    print("📦 ЗАГРУЗКА ТОВАРОВ FALMEC В БАЗУ ДАННЫХ")
    print("=" * 70)
    ...
    try:
        # Получаем или создаем бренд Falmec
        brand = db.query(Brand).filter(Brand.id == FALMEC_BRAND_ID).first()
        if not brand:
            brand = Brand(id=FALMEC_BRAND_ID, name='Falmec')
            db.add(brand)
            db.commit()
            db.refresh(brand)
            print(f"✅ Создан бренд: Falmec (id: {brand.id})")
        else:
            print(f"✅ Бренд найден: Falmec (id: {brand.id})")
        ...
        for idx, row in df.iterrows():
            ...
            if not model or pd.isna(model):
                print(f"⚠️ Строка {row.get('excel_row')}: пропущено (нет модели)")
                skipped_count += 1
                continue
            ...
            if sheet_num in photos_by_sheet:
                ...
                # 2. Fallback: картинка могла "уехать" на строку заголовка или на строку ниже
            ...
            if photo_path:
                print(f"  📷 Лист {sheet_num}, строка {xml_row}: {model[:40]}... -> ЕСТЬ фото")
            else:
                print(f"  ⚠️ Лист {sheet_num}, строка {xml_row}: {model[:40]}... -> НЕТ фото")

            # Получаем остальные поля
            manufacturer_code = None
            for col_name in ['Код производителя', 'код производителя']:
                if col_name in row and pd.notna(row[col_name]):
                    manufacturer_code = clean_string(row[col_name])
                    if manufacturer_code:
                        break

            mounting_type = None
            for col_name in ['Тип монтажа', 'тип монтажа']:
                if col_name in row and pd.notna(row[col_name]):
                    mounting_type = clean_string(row[col_name])
                    if mounting_type:
                        break

            color = None
            for col_name in ['Цвет', 'цвет']:
                if col_name in row and pd.notna(row[col_name]):
            ...
            width_cm = None
            for col_name in ['Ширина, см', 'ширина, см', 'Ширина', 'ширина']:
                if col_name in row and pd.notna(row[col_name]):
                    width_cm = clean_width(row[col_name])
                    if width_cm:
                        break

            performance_m3h = None
            for col_name in ['Производительность, м3/час', 'производительность, м3/час']:
                if col_name in row and pd.notna(row[col_name]):
                    performance_m3h = clean_integer(row[col_name])
                    if performance_m3h:
                        break

            min_noise_db = None
            for col_name in ['Мин. уровень шума, Дб', 'мин. уровень шума, дб']:
                if col_name in row and pd.notna(row[col_name]):
                    min_noise_db = clean_integer(row[col_name])
                    if min_noise_db:
                        break

            supply_program = None
            for col_name in ['Программа поставок', 'программа поставок']:
                if col_name in row and pd.notna(row[col_name]):
                    supply_program = clean_string(row[col_name])
                    if supply_program:
                        break

            control_type = None
            for col_name in ['Управление', 'управление']:
                if col_name in row and pd.notna(row[col_name]):
                    control_type = clean_string(row[col_name])
                    if control_type:
                        break

            price_retail = None
            for col_name in ['Цена розница, РУБ.', 'цена розница, руб.']:
                if col_name in row and pd.notna(row[col_name]):
                    price_retail = clean_price(row[col_name])
                    if price_retail:
                        break
            ...
            product_data = {
                ...
                'color': color,
                'width_cm': width_cm,
                'performance_m3h': performance_m3h,
                'min_noise_db': min_noise_db,
                'supply_program': supply_program,
            ...
            if existing:
                ...
                new_product = FalmecProduct(**product_data)
                db.add(new_product)
                new_count += 1
        ...
        print(f"   ⏭️ Пропущено (дубликаты/пустые): {skipped_count}")
        print(f"   📦 Всего товаров в БД: {db.query(FalmecProduct).count()}")
        print(f"   📷 Товаров с фото: {db.query(FalmecProduct).filter(FalmecProduct.main_image.isnot(None)).count()}")
...
if __name__ == "__main__":
    print("\n" + "=" * 70)
    print("🚀 ЗАПУСК ЗАГРУЗКИ ТОВАРОВ FALMEC")
    print("=" * 70)
