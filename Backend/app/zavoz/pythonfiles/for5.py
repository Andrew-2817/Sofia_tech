#!/usr/bin/env python3
"""Извлечение изображений из Excel файла и загрузка товаров Dedietrich в БД"""

import sys
import re
import zipfile
import shutil
from pathlib import Path
from PIL import Image
import io
import xml.etree.ElementTree as ET

# Пути
BASE_DIR = Path('/home/raul/projects/sofa2/Sofia_tech')
EXCEL_FILE = BASE_DIR / 'Backend' / 'app' / 'zavoz' / 'exelfiles' / 'файл_товары_5.xlsx'
PHOTOS_DIR = BASE_DIR / 'Backend' / 'app' / 'zavoz' / 'photos'
UPLOADS_DIR = BASE_DIR / 'Backend' / 'static' / 'uploads' / 'products'

# Добавляем путь для импорта
sys.path.insert(0, str(BASE_DIR))

from Backend.app.database import SessionLocal
from Backend.app.models.product_dedietrich import DedietrichProduct
from Backend.app.models.brand import Brand
from Backend.app.models.category import Category
from sqlalchemy import text
import pandas as pd

# ID бренда Dedietrich
DEDIETRICH_BRAND_ID = 5

def clean_price(price_str):
    """Очистка цены от символов"""
    if pd.isna(price_str):
        return None
    price_str = str(price_str).strip()
    price_str = price_str.replace('₽', '').replace('руб', '').replace(' ', '').replace(',', '.')
    price_str = re.sub(r'[^\d.]', '', price_str)
    try:
        return float(price_str)
    except:
        return None

def clean_string(value):
    if pd.isna(value):
        return None
    return str(value).strip()

def clean_float(value):
    if pd.isna(value):
        return None
    try:
        if isinstance(value, str):
            value = value.replace(',', '.')
        return float(value)
    except:
        return None

def convert_to_jpg(image_data, quality=85):
    """Конвертирует изображение в JPG формат, при ошибке сохраняет как PNG"""
    try:
        img = Image.open(io.BytesIO(image_data))
        if img.mode in ('RGBA', 'LA', 'P'):
            rgb_img = Image.new('RGB', img.size, (255, 255, 255))
            rgb_img.paste(img, mask=img.split()[-1] if img.mode == 'RGBA' else None)
            img = rgb_img
        elif img.mode != 'RGB':
            img = img.convert('RGB')
        output = io.BytesIO()
        img.save(output, format='JPEG', quality=quality, optimize=True)
        return output.getvalue(), 'jpg'
    except Exception as e:
        print(f"      ⚠️ Ошибка конвертации в JPG: {e}. Попытка сохранить как PNG...")
        # Попытка сохранить оригинал, если это PNG или другой формат, поддерживаемый PIL
        try:
            img = Image.open(io.BytesIO(image_data))
            output = io.BytesIO()
            img.save(output, format='PNG')
            return output.getvalue(), 'png'
        except:
            pass
        return image_data, 'png'

def cleanup_products_table():
    """Очистка таблицы products_dedietrich перед загрузкой"""
    print("=" * 70)
    print("🗑️ ОЧИСТКА ТАБЛИЦЫ products_dedietrich")
    print("=" * 70)

    db = SessionLocal()
    try:
        count_before = db.query(DedietrichProduct).count()
        print(f"📊 Записей до очистки: {count_before}")

        deleted = db.query(DedietrichProduct).delete()
        db.commit()

        try:
            db.execute(text("ALTER SEQUENCE products_dedietrich_id_seq RESTART WITH 1"))
            db.commit()
        except:
            pass

        print(f"✅ Удалено записей: {deleted}")
        print(f"📊 Записей после очистки: {db.query(DedietrichProduct).count()}")

    except Exception as e:
        print(f"❌ Ошибка очистки: {e}")
        db.rollback()
    finally:
        db.close()

def find_header_row(file_path):
    """Находит строку с заголовками в Excel файле"""
    try:
        df_preview = pd.read_excel(file_path, header=None, nrows=20)

        for idx, row in df_preview.iterrows():
            row_values = [str(v).lower() for v in row.values if pd.notna(v)]

            if ('id' in row_values or 'модель' in row_values or
                'наименование' in row_values or 'ррц' in row_values):
                print(f"📌 Найдена строка с заголовками: {idx}")
                return idx

        print("⚠️ Строка с заголовками не найдена, используем первую строку")
        return 0
    except Exception as e:
        print(f"⚠️ Ошибка поиска заголовков: {e}")
        return 0

def extract_all_images_with_order():
    """Извлекает ВСЕ изображения в порядке их следования через XML (как в Kuppersbusch)"""
    print("=" * 70)
    print("📸 ИЗВЛЕЧЕНИЕ ИЗОБРАЖЕНИЙ ПО ПОРЯДКУ (DEDIETRICH)")
    print("=" * 70)

    if not EXCEL_FILE.exists():
        print(f"❌ ФАЙЛ НЕ НАЙДЕН: {EXCEL_FILE}")
        return {}

    print(f"✅ Файл найден: {EXCEL_FILE.name}")

    PHOTOS_DIR.mkdir(parents=True, exist_ok=True)
    UPLOADS_DIR.mkdir(parents=True, exist_ok=True)

    # Очищаем старые фото (учитываем любые расширения: jpg, png и т.д.)
    for old_file in PHOTOS_DIR.glob("5.*.*"):
        old_file.unlink()
    for old_file in UPLOADS_DIR.glob("5.*.*"):
        old_file.unlink()
    print("📁 Старые фото Dedietrich удалены")

    photo_by_row = {}

    try:
        with zipfile.ZipFile(EXCEL_FILE, 'r') as zip_ref:
            # Находим все файлы изображений
            image_files = {}
            media_files = [f for f in zip_ref.namelist() if f.startswith('xl/media/')]
            print(f"📸 Найдено медиа файлов в архиве: {len(media_files)}")

            for file_name in media_files:
                file_lower = file_name.lower()
                if any(ext in file_lower for ext in ['.png', '.jpg', '.jpeg', '.gif', '.emf', '.wmf']):
                    img_name = Path(file_name).stem
                    img_id = re.search(r'image(\d+)', img_name, re.IGNORECASE)
                    img_num = int(img_id.group(1)) if img_id else 0
                    image_files[file_name] = {'num': img_num, 'name': file_name}

            print(f"📸 Обработано изображений: {len(image_files)}")

            # Читаем XML для получения порядка фото
            drawing_files = [f for f in zip_ref.namelist() if 'xl/drawings/drawing' in f and f.endswith('.xml')]
            print(f"📄 Найдено drawing файлов: {len(drawing_files)}")

            # Собираем все фото с их позициями (XML row)
            photos_with_positions = []

            for drawing_file in drawing_files:
                try:
                    xml_content = zip_ref.read(drawing_file)
                    root = ET.fromstring(xml_content)

                    namespaces = {
                        'xdr': 'http://schemas.openxmlformats.org/drawingml/2006/spreadsheetDrawing',
                        'a': 'http://schemas.openxmlformats.org/drawingml/2006/main',
                        'r': 'http://schemas.openxmlformats.org/officeDocument/2006/relationships'
                    }

                    # ИСПРАВЛЕНИЕ 1: Ищем ВСЕ типы якорей (twoCellAnchor и oneCellAnchor)
                    anchors = root.findall('.//xdr:twoCellAnchor', namespaces) + \
                              root.findall('.//xdr:oneCellAnchor', namespaces)

                    for anchor in anchors:
                        pic = anchor.find('.//xdr:pic', namespaces)
                        if pic is not None:
                            blip = pic.find('.//a:blip', namespaces)
                            if blip is not None:
                                r_id = blip.get('{http://schemas.openxmlformats.org/officeDocument/2006/relationships}embed')
                                from_elem = anchor.find('.//xdr:from', namespaces)
                                if from_elem is not None:
                                    row_elem = from_elem.find('.//xdr:row', namespaces)
                                    if row_elem is not None:
                                        xml_row = int(row_elem.text)
                                        photos_with_positions.append({
                                            'r_id': r_id,
                                            'xml_row': xml_row
                                        })
                except Exception as e:
                    print(f"  ⚠️ Ошибка парсинга {drawing_file}: {e}")

            # Сортируем по XML row
            photos_with_positions.sort(key=lambda x: x['xml_row'])
            print(f"📸 Найдено позиций фото в XML: {len(photos_with_positions)}")

            # Читаем связи rId -> имя файла
            rels_files = [f for f in zip_ref.namelist() if 'xl/drawings/_rels/drawing' in f and f.endswith('.rels')]
            rid_to_filename = {}

            for rels_file in rels_files:
                try:
                    xml_content = zip_ref.read(rels_file)
                    root = ET.fromstring(xml_content)
                    for rel in root.findall('.//{http://schemas.openxmlformats.org/package/2006/relationships}Relationship'):
                        rel_id = rel.get('Id')
                        target = rel.get('Target')
                        if target and ('media' in target or 'image' in target):
                            filename = Path(target).name
                            rid_to_filename[rel_id] = filename
                except Exception as e:
                    print(f"  ⚠️ Ошибка парсинга {rels_file}: {e}")

            print(f"🔗 Найдено связей rId: {len(rid_to_filename)}")

            # Извлекаем фото по их реальным позициям
            print(f"\n📸 Извлечение фото по реальным позициям...")

            for photo_info in photos_with_positions:
                rid = photo_info['r_id']
                xml_row = photo_info['xml_row']

                if rid in rid_to_filename:
                    img_filename = rid_to_filename[rid]
                    img_path_in_zip = None
                    for file_path in image_files.keys():
                        if Path(file_path).name == img_filename:
                            img_path_in_zip = file_path
                            break

                    if img_path_in_zip:
                        img_data = zip_ref.read(img_path_in_zip)
                        if len(img_data) > 100:
                            converted_data, ext = convert_to_jpg(img_data)

                            # ВАЖНО: xml_row - это 0-индекс, Excel строки начинаются с 1
                            # Но у нас первая строка - заголовки (строка 0 в индексе)
                            # Поэтому номер строки с данными = xml_row + 1 (так как заголовки на строке 0)
                            excel_row_number = xml_row + 1

                            # Проверяем: если xml_row = 0, это строка с заголовками - пропускаем
                            if xml_row == 0:
                                print(f"  ⚠️ Пропуск: фото для строки заголовков (row=0)")
                                continue

                            # ИСПРАВЛЕНИЕ 2: ДИНАМИЧЕСКОЕ РАСШИРЕНИЕ ФАЙЛА
                            img_name = f"5.{excel_row_number}.{ext}"
                            img_path = PHOTOS_DIR / img_name
                            with open(img_path, 'wb') as f:
                                f.write(converted_data)

                            dest_path = UPLOADS_DIR / img_name
                            shutil.copy2(img_path, dest_path)

                            photo_by_row[excel_row_number] = {
                                'file_name': img_name,
                                'path': f"/uploads/products/{img_name}",
                                'row': excel_row_number,
                                'xml_row': xml_row
                            }
                            print(f"  ✅ Строка {excel_row_number} -> {img_name}")
                    else:
                        print(f"  ⚠️ Файл не найден в архиве: {img_filename}")
                else:
                    print(f"  ⚠️ Нет связи rId в rels файле для: {rid}")

    except Exception as e:
        print(f"❌ Ошибка при обработке: {e}")
        import traceback
        traceback.print_exc()
        return {}

    print(f"\n📊 ИТОГО ИЗВЛЕЧЕНО: {len(photo_by_row)} изображений")

    if photo_by_row:
        print("\n📋 Соответствие строк и фото (первые 20):")
        for row_num in sorted(photo_by_row.keys())[:20]:
            print(f"   Строка {row_num} -> {photo_by_row[row_num]['file_name']}")

    return photo_by_row

def load_products_to_db(photo_by_row):
    """Загрузка товаров Dedietrich в базу данных"""
    print("\n" + "=" * 70)
    print("📦 ЗАГРУЗКА ТОВАРОВ DEDIETRICH В БАЗУ ДАННЫХ")
    print("=" * 70)

    if not EXCEL_FILE.exists():
        print(f"❌ Файл не найден: {EXCEL_FILE}")
        return

    # Находим строку с заголовками
    header_row = find_header_row(EXCEL_FILE)

    # Читаем Excel файл с указанием строки заголовков
    df = pd.read_excel(EXCEL_FILE, header=header_row)

    print(f"📊 Строка заголовков: {header_row}")
    print(f"📊 Найдено строк данных в Excel: {len(df)}")
    print(f"📊 Доступные колонки: {list(df.columns)}")

    db = SessionLocal()

    try:
        # Получаем или создаем бренд Dedietrich
        brand = db.query(Brand).filter(Brand.id == DEDIETRICH_BRAND_ID).first()
        if not brand:
            brand = Brand(id=DEDIETRICH_BRAND_ID, name='Dedietrich')
            db.add(brand)
            db.commit()
            db.refresh(brand)
            print(f"✅ Создан бренд: Dedietrich (id: {brand.id})")
        else:
            print(f"✅ Бренд найден: Dedietrich (id: {brand.id})")

        new_count = 0
        updated_count = 0
        skipped_count = 0

        # Проходим по всем строкам Excel
        for idx, row in df.iterrows():
            # Реальный номер строки в Excel (с учетом заголовков)
            excel_row_num = idx + header_row + 2

            # Получаем наименование товара
            name = None
            for col_name in ['Наименование', 'наименование', 'Name', 'name']:
                if col_name in row:
                    name = clean_string(row[col_name])
                    if name:
                        break

            if not name or pd.isna(name):
                print(f"⚠️ Строка {excel_row_num}: пропущено (нет наименования)")
                skipped_count += 1
                continue

            # Получаем фото
            photo_path = None
            if excel_row_num in photo_by_row:
                photo_path = photo_by_row[excel_row_num]['path']
                print(f"  📷 Строка {excel_row_num}: {name[:30]}... -> ЕСТЬ фото")
            else:
                print(f"  ⚠️ Строка {excel_row_num}: {name[:30]}... -> НЕТ фото")

            # Получаем данные из Excel
            category_id = None
            for col_name in ['category_id', 'Category ID']:
                if col_name in row:
                    val = clean_float(row[col_name])
                    if val:
                        category_id = int(val)
                        break

            model = clean_string(row.get('Модель'))
            line = clean_string(row.get('Линейка'))
            specifications = clean_string(row.get('Характеристики'))
            color = clean_string(row.get('Цвет'))
            price_public = clean_price(row.get('РРЦ, руб'))
            comment = clean_string(row.get('Комментарий'))

            product_data = {
                'category_id': category_id,
                'brand_id': brand.id,
                'main_image': photo_path,
                'model': model,
                'name': name,
                'line': line,
                'specifications': specifications,
                'color': color,
                'price_public': price_public,
                'comment': comment
            }

            # Проверяем, существует ли товар с такой моделью
            existing = None
            if model:
                existing = db.query(DedietrichProduct).filter(
                    DedietrichProduct.model == model
                ).first()

            if existing:
                for key, value in product_data.items():
                    if value is not None:
                        setattr(existing, key, value)
                updated_count += 1
            else:
                new_product = DedietrichProduct(**product_data)
                db.add(new_product)
                new_count += 1

        db.commit()

        print("\n" + "=" * 70)
        print("📊 РЕЗУЛЬТАТ:")
        print(f"   ✅ Добавлено новых товаров: {new_count}")
        print(f"   🔄 Обновлено товаров: {updated_count}")
        print(f"   ⏭️ Пропущено: {skipped_count}")
        print(f"   📷 Всего фото в папке: {len(photo_by_row)}")
        print(f"   📦 Всего товаров в БД: {db.query(DedietrichProduct).count()}")
        print(f"   📷 Товаров с фото: {db.query(DedietrichProduct).filter(DedietrichProduct.main_image.isnot(None)).count()}")

    except Exception as e:
        print(f"❌ ОШИБКА: {e}")
        import traceback
        traceback.print_exc()
        db.rollback()
    finally:
        db.close()

if __name__ == "__main__":
    print("\n" + "=" * 70)
    print("🚀 ЗАПУСК ЗАГРУЗКИ ТОВАРОВ DEDIETRICH")
    print("=" * 70)

    # Очищаем таблицу
    cleanup_products_table()

    # Извлекаем фото через XML
    photos_by_row = extract_all_images_with_order()

    if photos_by_row:
        print(f"\n✅ Найдено {len(photos_by_row)} фото")
    else:
        print("\n⚠️ ВНИМАНИЕ: Фото не найдены!")

    # Загружаем товары в БД
    load_products_to_db(photos_by_row)

    print("\n" + "=" * 70)
    print("🏁 ЗАВЕРШЕНО")
    print("=" * 70)