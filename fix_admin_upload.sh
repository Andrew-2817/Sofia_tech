#!/bin/bash
set -e
echo "🔧 Исправляю admin.py: добавляю загрузку фото через FileField..."
...
# ---------- Базовый класс с общими методами загрузки фото ----------
class BaseProductAdmin(ModelView):
    ...
    async def on_model_change(self, data, model, is_created, request):
        if request.method == "POST":
            ...
            if "main_image_file" in files and files["main_image_file"]:
                upload_file = files["main_image_file"]
...
# ---------- Бренды (наследуют BaseProductAdmin) ----------
...
# ---------- Остальные разделы (Brand, Category, Order, User) ----------
...
echo "✅ admin.py успешно заменён (с кнопкой загрузки фото)"
